...
export default function AdminSalesQuotationsPage() {
  ...
  const handleConvertToInvoice = async (quotation: Quotation) => {
    try {
      ...
      const updatedQuotations = quotations.map(q => 
        q.id === quotation.id ? { ...q, status: 'converted' as const, updatedAt: new Date().toISOString() } : q
      ) as Quotation[];
      setQuotations(updatedQuotations);
  ...
  const handleSendQuotation = async (quotation: Quotation) => {
    try {
      ...
      const updatedQuotations = quotations.map(q => 
        q.id === quotation.id ? { ...q, status: 'sent' as const, updatedAt: new Date().toISOString() } : q
      ) as Quotation[];
      setQuotations(updatedQuotations);
